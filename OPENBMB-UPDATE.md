...
The public package includes171 offline tests and a fictional no-model daily demo.
The separate private integration passes242 tests in total. The editorial update
...
A subsequent current-data test sampled nine recent discoveries from six sources.
Two fresh local requests produced verified GitHub article selections in about
40–42 seconds each; seven other items remained labeled source links. The two-part
Telegram test was confirmed delivered without consuming the scheduled archive.
The original articles had been used before, so this is not an unseen holdout test.
